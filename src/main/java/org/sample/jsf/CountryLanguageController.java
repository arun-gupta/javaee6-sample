...
import org.sample.jsf.util.JsfUtil;
import org.sample.jsf.util.PaginationHelper;
import org.sample.CountryLanguageFacade;
...
public class CountryLanguageController implements Serializable {
    ...
    @EJB
    private org.sample.CountryLanguageFacade ejbFacade;
    ...
    public CountryLanguage getSelected() {
        if (current == null) {
            ...
            current.setCountryLanguagePK(new org.sample.entities.CountryLanguagePK());
            selectedItemIndex = -1;
        }
        return current;
    }
    ...
    public static class CountryLanguageControllerConverter implements Converter {
        ...
        org.sample.entities.CountryLanguagePK getKey(String value) {
            org.sample.entities.CountryLanguagePK key;
            String values[] = value.split(SEPARATOR_ESCAPED);
            key = new org.sample.entities.CountryLanguagePK();
            key.setCountryCode(values[0]);
            key.setLanguage(values[1]);
            return key;
        }

        String getStringKey(org.sample.entities.CountryLanguagePK value) {
